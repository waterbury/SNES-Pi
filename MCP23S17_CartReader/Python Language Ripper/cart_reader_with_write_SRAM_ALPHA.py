...
def writeSRAMfunc(SRAMsize, isLowROM, fileName):
 ...
 if isLowROM == 1:
  ...
  startAddr = 0x0000
  endAddr   = 0x7FFF
  controls = 0x03 # RESET + /RD high
  
 else: #Else, is HiROM
  startBank = 0x30
  startAddr = 0x6000
  endAddr   = 0x7FFF
  controls = 0x0B # RESET + /RD + /CS high
  # GPA7: /IRQ | GPA4: CART MOSFET | GPA3: /CS | GPA2: /WR | GPA1: /RESET  | GPA0: /RD
 ...
 SRAMfile = open(fileName, 'r+')

 if fileSize != SRAMsize: 
  print "SRAMsize does not match file size. Not Writing!"
 else:
  cart.write_byte_data(_IOControls,GPIOA,controls)
  # GPA7: /IRQ | GPA4: CART MOSFET | GPA3: /CS | GPA2: /WR | GPA1: /RESET  | GPA0: /RD
...
#  cart.write_byte_data(_IOControls,GPIOA,controls)
  # GPA7: /IRQ | GPA4: CART MOSFET | GPA3: /CS | GPA2: /WR | GPA1: /RESET  | GPA0: /RD
  cart.write_byte_data(_SNESBankAndData,GPIOB,0xFF)
  time.sleep(1)

  while SRAMsize > currentByte:  
   SRAMfile.seek(currentByte)
   value = ord( SRAMfile.read(1) )
   #value = 0xE8
   cart.write_byte_data(_SNESBankAndData,GPIOB,value) 
   #if currentByte <20:
   # time.sleep(.1)
   currentByte += 1
 ...
 # GPA7: /IRQ | GPA4: CART MOSFET | GPA3: /CS | GPA2: /WR | GPA1: /RESET  | GPA0: /RD
 print str(currentByte) + " SRAM bytes written"
 SRAMfile.close()
...
try:
 opts, args = getopt.getopt(sys.argv[1:],"Ssz:w:d:",["directory="])
...
if isValid == 1:
 ...
 if writeSRAM == 1:
  timeStart = time.time()
  print "Writing save data to cart"
  writeSRAMfunc(convertedSRAMsize,isLowROM,SRAMfilename)
  timeEnd = time.time()
  print "It took " + str(timeEnd - timeStart) + "seconds to write SRAM Data"
